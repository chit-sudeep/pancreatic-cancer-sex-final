...
# Section 7: Tumor Sex Subset Function
...
vsdFeMaleTumorCopy <- vsdFeMaleTumor
colnames(vsdFeMaleTumorCopy) <- paste0("tumor", colnames(vsdFeMaleTumorCopy))
vsdFeMaleNormalCopy <- vsdFeMaleNormal
colnames(vsdFeMaleNormalCopy) <- paste0("norma", colnames(vsdFeMaleNormalCopy))

vsdDataFeMale <- merge(x=cbind(gene=rownames(vsdFeMaleTumorCopy),vsdFeMaleTumorCopy),y=cbind(gene=rownames(vsdFeMaleNormalCopy),vsdFeMaleNormalCopy),by="gene",all.x=TRUE, all.y=TRUE)
vsdDataFeMale <- na.omit(vsdDataFeMale)
rownames(vsdDataFeMale) <- vsdDataFeMale[,1]
vsdDataFeMale <- vsdDataFeMale[-c(1)]

namesFeMale = colnames(vsdDataFeMale)
groupFeMale = substr(namesFeMale, 1, 5)
vsdDataFeMale$samples$group = groupFeMale #Assign samples to appropriate group
designFeMale = model.matrix(~0 + groupFeMale)
colnames(designFeMale) = gsub("group", "", colnames(designFeMale))
contr.matrixFeMale = makeContrasts(TumorFeMaleandNormalFeMale = FeMaletumor-FeMalenorma, levels = colnames(designFeMale))

# Fit a linear model using weighted least square for each gene

vfitDataFeMale = lmFit(vsdDataFeMale[,1:ncol(vsdDataFeMale)-1], designFeMale)

vfitDataFeMaleContr = contrasts.fit(vfitDataFeMale, contrasts = contr.matrixFeMale)

#  Perform empirical Bayes smoothing of standard errors
head(coef(vfitDataFeMale))
efitDataFeMale = eBayes(vfitDataFeMaleContr)

ttvpFeMale <- topTableAndVolcanoPlotGenerator(efitDataFeMale, 5, 0.05)

topTableFeMale <- ttvpFeMale[[1]]
volcanPlotFeMale <- ttvpFeMale[[2]]
geneComparisonListFeMaleAll <- ttvpFeMale[[3]]
geneComparisonListFeMaleUP <- ttvpFeMale[[4]]
geneComparisonListFeMaleDOWN <- ttvpFeMale[[5]]
topTableFeMale1 <- ttvpFeMale[[6]]
