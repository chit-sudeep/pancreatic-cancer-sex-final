...
# Section 6: Normal Sex Subset Function
...
# Section 10: Differential Gene Expression between Male Pancreatic Normal Tissue and Female Pancreatic Normal Tissue

rse_geneFeMaleNormal <- normalsexsubsetter(rse_geneNormalData, "F")[[1]]
...
for (i in 1:length(colnames(vsdFeMaleNormal))){
  colnames(vsdFeMaleNormal)[i] <- paste("genderf; ", colnames(vsdFeMaleNormal)[i])
}
for (i in 1:length(colnames(vsdMaleNormal))){
  colnames(vsdMaleNormal)[i] <- paste("genderm; ", colnames(vsdMaleNormal)[i])
}

vsdNormalData2 <- merge(x=cbind(gene=rownames(vsdFeMaleNormal),vsdFeMaleNormal),y=cbind(gene=rownames(vsdMaleNormal),vsdMaleNormal),by="gene",all.x=TRUE, all.y=TRUE)
vsdNormalData2 <- na.omit(vsdNormalData2)
rownames(vsdNormalData2) <- vsdNormalData2[,1]
vsdNormalData2 <- vsdNormalData2[-c(1)]

namesNormalFeMaleandNormalMale = colnames(vsdNormalData2)
groupNormalFeMaleandNormalMale = substr(namesNormalFeMaleandNormalMale, 1, 7)
vsdNormalData2$samples$group = groupNormalFeMaleandNormalMale #Assign samples to appropriate group
designNormalFeMaleandNormalMale = model.matrix(~0 + groupNormalFeMaleandNormalMale)
colnames(designNormalFeMaleandNormalMale) = gsub("group", "", colnames(designNormalFeMaleandNormalMale))
contr.matrixNormalFeMaleandNormalMale = makeContrasts(NormalFeMaleandNormalMale = NormalFeMaleandNormalMalegenderf-NormalFeMaleandNormalMalegenderm, levels = colnames(designNormalFeMaleandNormalMale))

# Fit a linear model using weighted least square for each gene

vfitNormalFeMaleandNormalMale = lmFit(vsdNormalData2[,1:ncol(vsdNormalData2)-1], designNormalFeMaleandNormalMale)

vfitNormalFeMaleandNormalMaleContr = contrasts.fit(vfitNormalFeMaleandNormalMale, contrasts = contr.matrixNormalFeMaleandNormalMale)
...
geneComparisonListNormalAll <- ttvpNormal[[3]]
geneComparisonListNormalUP <- ttvpNormal[[4]]
geneComparisonListNormalDOWN <- ttvpNormal[[5]]
topTableNormal1 <- ttvpNormal[[6]]
...
NormalFeMalerse <- normalsexsubsetter2(Normaloutput[[1]], "F")[[1]]
NormalFeMalevsd <- sexsubsettervsd2(NormalFeMalerse, Normaloutput[[2]])

NormalMaleMatrix <- as.data.frame(t(NormalMalevsd))
NormalFeMaleMatrix <- as.data.frame(t(NormalFeMalevsd))
...
lincrnavector <- as.vector(rownames(vsdNormalDatalincRNA))
